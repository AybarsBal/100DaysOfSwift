...
class ImageViewController: UIViewController {
	weak var owner: SelectionViewController? // to break strong reference cycle
	var image: String? // challenge 1(Bonus)
	var animTimer: Timer? // challenge 1(Bonus)
    ...
    override func viewDidLoad() {
        ...
        guard let path = Bundle.main.path(forResource: image, ofType: nil) else { return } // challenge 1
//		let original = UIImage(named: image)!
        guard let original = UIImage(contentsOfFile: path) else { return } // challenge 1
    ...
    // stops timer to force it to release its strong reference
	...
	override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        guard let image = image else { return } // challenge 1(bonus)
