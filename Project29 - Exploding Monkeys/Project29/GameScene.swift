...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    weak var viewController: GameViewController! // reference to the gameViewController
    ...
    var isGameOver = false // challenge 1
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if firstNode.name == "banana" && secondNode.name == "player1" {
            viewController.player2Score += 1 // challenge 1
            destroy(player: player1)
        }
        if firstNode.name == "banana" && secondNode.name == "player2" {
            viewController.player1Score += 1 // challenge 1
    ...
    func destroy(player: SKSpriteNode) {
        ...
        // challenge 1
        if viewController.player1Score == 3 || viewController.player2Score == 3 { isGameOver = true }
        if !isGameOver {
    ...
    // challenge 3
    func addWind() {
        let wind = Double.random(in: -2...2)
        physicsWorld.gravity = CGVector(dx: wind, dy: -9.8)
        
        var windText = ""
        if wind > 0 {
            windText = "Wind: \(String(format: "%.2f", abs(wind))) ->>"
        } else {
            windText = "Wind: <<- \((String(format: "%.2f", abs(wind))))"
        }
        ...
        node.text = windText
        node.position = CGPoint(x: 512, y: 650)
        addChild(node)
    }
}
